...
struct Graphs: View {
    ...
    func ChartView() -> some View {
        ...
        .chartForegroundStyleScale(range: [Color.green.gradient, Color.blue.gradient])
    }
    ...
    func createChartGroup() {
        Task.detached(priority: .high){
            ...
            let groupedByDate = Dictionary(grouping: transactions) {
                transaction in
            ...
            let chartGroups = sortedGroups.compactMap {
                dict -> ChartGroup? in
        ...
        var body: some View {
            ScrollView(.vertical) {
                LazyVStack(spacing: 15) {
                    //income
                    Section {
                        FilterTransactionView(startDate: month.startofMonth, endDate: month.endofMonth, category: .income) {
                            transactions in
                    ...
                    //expense
                    Section {
                        FilterTransactionView(startDate: month.startofMonth, endDate: month.endofMonth, category: .expense) {
                            transactions in
                            ForEach(transactions) { transaction in
                                NavigationLink(value: transaction){
                                    TransactionCardView(transaction: transaction)
